...
#'
#' @param dataframe a file of streamflow data with columns Date and Flow
#' @param threshold a value for the threshold above which is a flood peak (POT - points over threshold)
#'
#' @return a list containing:
#' 	\code{POTevents} a dataframe contining details of the events
#'	\code{events} an array that has 0 when below the threshold and 1 when above.
#'	\code{event_num} an array that has 0 when the flow was below a threshold and an index of the events
#'	when the threshold was exceeded. i.e. 1,2,3, etc whenever the event number is switched to 0
#'	\code{st_date} start date of events
#'	\code{case} a list of the flows in each individual event (see details for more information)
#'	
#'	@details 
#'	The POTevents dataframe contains five columns: st_date (starting date), max_date (date of maximum in the event),
#'	max (maximum discharge), volume (volume of the event), and duration (in days).
#'	
#'	case lists the flows during an event and also for three preceeding and subsequent days. These lists 
#'	themselves range from  seven to n days in length.
#'
#' @references 
...
get_peaks <- function (dataframe, threshold) {
  ...
  for (k in 1:length(data)){
    ...
    {                     
      ...
      if (k >3) {	
        event_num[k-1] <- index	;flow[3] <-data[k-1] 
        event_num[k-2] <- index; flow[2] <-data[k-2]
        event_num[k-3] <- index; flow[1] <-data[k-3]
      }
    ...
    {      
      event_num[k]   <-index; flow[k-ii]  <-data[k]   ## add next  3 days as end of event
